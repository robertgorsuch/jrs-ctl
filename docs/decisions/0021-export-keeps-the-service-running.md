...
4. Unchanged, on purpose:
   ...
   - The import's pre-import snapshot still stops the service. The import stops it right after anyway, and changing it would make an interrupted import journaled by an earlier jrsctl rebuild into a different plan. *Superseded by ADR-0040 (2026-09-25): the snapshot runs with the server up, so a vendor import has one outage; arguments stored without `snapshotStopsService` still rebuild the stopping plan, so a journaled run is recovered with its own steps.*
