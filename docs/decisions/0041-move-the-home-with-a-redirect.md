...
- When the system home exists but this user cannot write it (the refusal of spec §5.1, #50), only a redirect in the system home itself is followed, written there by an account that can; a redirect in the per-user fallback is ignored and the refusal stands, so `home set` cannot be used to give one operator a second journal and run lock (review of #169). `home show` and `home set` act on the system home in that case, so an unprivileged `set` fails asking for elevation.
