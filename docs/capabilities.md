...
- Two vendor modes, `newdb` (drop and recreate the repository database from the full export; the default) and `samedb` (migrate the schema in place); `samedb` runs only with an explicit, audited confirmation that a database backup exists and its rollback restores files only (ADR-0012); `newdb` asks nothing, since its own point-B export is the backup, and `upgrade rollback --restore-database` rebuilds the old database from it (ADR-0029).
