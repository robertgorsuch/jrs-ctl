...
| Secrets in output | Logs, `--json`, SSE payloads, support bundles, error messages | One `Redactor` masks every configured secret (raw, Base64, URL-encoded, JSON-string-escaped) and the well-known patterns (`password=`, `Authorization:`, `JSESSIONID`, `Bearer`, the console token) on every stream that leaves the process |
