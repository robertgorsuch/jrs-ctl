...
|---|---|---|---|
| **Operator** | The administrator of a JasperReports Server | Receives a bundle ZIP and runs `jrsctl hotfix verify`, `hotfix apply`, `hotfix list` and, if needed, `hotfix rollback` | The bundle. Bundles from Jaspersoft verify against the publisher key built into jrsctl, so nothing else is needed. For a bundle signed by anyone else, that author's public key, trusted once with `jrsctl keys add <name> <file>` |
