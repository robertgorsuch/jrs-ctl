...
### `jrsctl import <archive> [--update] [--skip-user-update] [--access-events] [--audit-events] [--monitoring] [--settings] [--skip-themes] [--broken-dependencies fail|skip|include] [--source-keystore <path>] [--source-keystore-password-ref <ref>] [--key-alias <alias>] [--strategy rest|vendor] [--plan] [--yes] [--json]`
