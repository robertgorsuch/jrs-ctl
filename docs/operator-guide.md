...
4. **Restore or copy content.** Asks for the archive, says where the pre-import rollback copy goes (the resources the archive will touch, or the whole repository when the archive has no `.jrsctl.json` beside it, exported under the home's `snapshots/` first), then asks the import's options: replace existing resources (`--update`), whether to take the rollback copy (Enter takes it; only `n` passes `--no-snapshot`), skip themes (`--skip-themes`), what to do with broken dependencies (`--broken-dependencies fail|skip|include`), and the strategy (`--strategy rest|vendor`, Enter for automatic).
