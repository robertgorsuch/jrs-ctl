...
1. **Set up or change settings.** The current settings are listed first, numbered, with secrets as references (field test 3); with no configuration yet, detection runs straight away. *Change a setting* asks for a setting by its number or its name (Tab completes names), puts its current value on the line to edit, and runs `config set <key> <value>`; Enter on an unchanged value changes nothing, clearing it offers `config unset`, a password setting is asked for hidden by `config set` itself, and an unknown name or number is re-asked, not sent. *Where jrsctl keeps its state and backups* runs `home show` and offers to move the home with `home set`. *Detect the installation again and rewrite the settings* runs `init` (Enter searches for the installation; a directory that does not exist is asked again).
2. **Check server health**: `doctor`.
3. **Back up content.** *Everything* asks one question that decides the tool (field test 3): include scheduled report jobs and calendars? Yes runs buildomatic's `js-export` (`export --full-server`, which also carries users, roles, events and settings) and asks whether to stop the server while it runs (`--stop-service`); no runs the server's REST export (`export --strategy rest`, the server keeps running). *One folder* asks for the folder (`--uri`). The REST exports then ask whether to include users and roles, the access, audit and monitoring events, and the server settings. Every export asks whether to encrypt with the legacy import/export key (`deprecatedImportExportEncSecret`, `--portable`) so another server can import it.
4. **Restore or copy content.** Asks for the archive, says where the pre-import rollback copy goes (the resources the archive will touch, or the whole repository when the archive has no `.jrsctl.json` beside it, exported under the home's `snapshots/` first), then asks the import's options: replace existing resources (`--update`), whether to take the rollback copy (Enter takes it; only `n` passes `--no-snapshot`), skip themes (`--skip-themes`), what to do with broken dependencies (`--broken-dependencies fail|skip|include`), and the strategy (`--strategy rest|vendor`, Enter for automatic). The key the archive was encrypted with comes from its `.jrsctl.json` when there is one, and nothing is asked; without it the menu asks whether it was exported with the legacy import/export key (`--key-alias deprecatedImportExportEncSecret`), and any other answer is taken as the alias itself.
