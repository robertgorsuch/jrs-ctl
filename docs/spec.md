...
- `RecordRepositoryListing` (`backup.pre-import-listing`, right after the snapshot announcement and before a vendor snapshot stops the service) lists every URI under the snapshotted folders through `GET /rest_v2/resources?recursive=true` in pages and writes them to `runs/<runId>/pre-import-listing.txt`; a listing that cannot be taken fails the step before anything is imported (issue #100, ADR-0031).
