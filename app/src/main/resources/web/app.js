...
const OPS = {
  ...
  import: {
    ...
    fields: () => [
      ...
      field('Organisation', textInput('organization', { mono: true, placeholder: 'organization_1' }), 'Import into this organisation; its id should match the archive\'s.'),
      checkField('mergeOrganization', 'Merge when the organisation ids differ', 'The archive\'s users, roles and resources override same-named ones in the target organisation.'),
    ...
    args: (v) => ({ archive: v.archive, update: !!v.update, skipUserUpdate: !!v.skipUserUpdate, sourceKeystore: v.sourceKeystore || null,
      ...
      brokenDependencies: v.brokenDependencies || 'fail', keyAlias: v.keyAlias || null, organization: v.organization || null, mergeOrganization: !!v.mergeOrganization }),
  ...
  upgrade: {
    ...
    fields: () => [
      ...
      checkField('dbBackupConfirmed', 'I have a database backup (samedb only)', 'samedb migrates the schema in place and no export undoes that. newdb needs no confirmation: jrsctl exports the repository first and upgrade rollback --restore-database rebuilds the database from that export.'),
    ...
    validate: (v) => (!v.to || !v.package ? 'Enter the target version and the package path.' : v.mode === 'samedb' && !v.dbBackupConfirmed ? 'Confirm the database backup before a samedb upgrade; the in-place migration cannot be undone.' : null),
  },
};
